...
@pytest.fixture
def mangacrab_server():
    from komikku.servers.mangacrab import Mangacrab

    return Mangacrab()


@test_steps('get_latest_updates', 'get_most_populars', 'search', 'get_manga_data', 'get_chapter_data', 'get_page_image')
def test_mangacrab(mangacrab_server):
    # Get latest updates
    ...
    try:
        response = mangacrab_server.get_manga_data(dict(slug=slug))
    ...
    try:
        response = mangacrab_server.get_manga_chapter_data(slug, None, chapter_slug, None)
    ...
    try:
        response = mangacrab_server.get_manga_chapter_page_image(slug, None, chapter_slug, page)
    except Exception as e:
        response = None
        log_error_traceback(e)
