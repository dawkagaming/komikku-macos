...
@pytest.fixture
def nightscans_server():
    from komikku.servers.nightscans import Nightscans
...
def test_nightscans(nightscans_server):
    # Get latest updates
    ...
    try:
        response = nightscans_server.get_manga_data(dict(slug=slug))
    ...
    try:
        response = nightscans_server.get_manga_chapter_page_image(None, None, chapter_slug, page)
    except Exception as e:
        response = None
        log_error_traceback(e)
