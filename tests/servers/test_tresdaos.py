...
@pytest.fixture
def tresdaos_server():
    from komikku.servers.tresdaos import Tresdaos
...
def test_tresdaos(tresdaos_server):
    # Get latest updates
    ...
    try:
        response = tresdaos_server.get_manga_data(dict(slug=slug))
    ...
    try:
        response = tresdaos_server.get_manga_chapter_data(slug, None, chapter_slug, None)
    ...
    try:
        response = tresdaos_server.get_manga_chapter_page_image(slug, None, chapter_slug, page)
    except Exception as e:
        response = None
        log_error_traceback(e)
