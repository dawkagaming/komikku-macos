...
@pytest.fixture
def mangago_server():
    from komikku.servers.mangago import Mangago
...
def test_mangago(mangago_server):
    # Get latest updates
    print('Get latest updates')
    try:
        response = mangago_server.get_latest_updates()
    ...
    try:
        response = mangago_server.get_most_populars()
    except Exception as e:
        response = None
        log_error_traceback(e)
    ...
    print('Search')
    try:
        response = mangago_server.search(response[0]['name'])
    ...
    try:
        response = mangago_server.get_manga_chapter_page_image(None, None, None, page)
    except Exception as e:
        response = None
        log_error_traceback(e)
