...
def test_scanvf(scanvf_server):
    # Get latest updates
    print('Get latest updates')
    try:
        response = scanvf_server.get_latest_updates()
    ...
    try:
        response = scanvf_server.get_most_populars()
    except Exception as e:
        response = None
        log_error_traceback(e)
    ...
    try:
        ...
        response = scanvf_server.search(response[0]['name'])
    ...
    try:
        response = scanvf_server.get_manga_data(dict(slug=slug))
    ...
    try:
        response = scanvf_server.get_manga_chapter_page_image(None, None, chapter_slug, page)
    except Exception as e:
        response = None
        log_error_traceback(e)
