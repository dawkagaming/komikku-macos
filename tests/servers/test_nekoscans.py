...
@pytest.fixture
def nekoscans_server():
    from komikku.servers.nekoscans import Nekoscans
...
def test_nekoscans(nekoscans_server):
    ...
    try:
        response = nekoscans_server.get_manga_data(dict(slug=slug))
    ...
    try:
        response = nekoscans_server.get_manga_chapter_data(slug, None, chapter_slug, None)
    ...
    try:
        response = nekoscans_server.get_manga_chapter_page_image(slug, None, chapter_slug, page)
    except Exception as e:
        response = None
        log_error_traceback(e)
